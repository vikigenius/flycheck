...
(not unlike `use-package`).

While this makes for a less noisy syntax, it unfortunately prevents you from
...
``my-other-foobar-mode`` before the checker is defined.  In theory, this should
work, but in practice it won't, since the ``:modes`` property is auto-quoted by
`flycheck-define-checker`.  The issue arise not just with ``:modes``:, but with
almost all the other properties since they are also auto-quoted.

Ii you do find yourself in need to define such a checker, there is a solution
...
need to explicitly define the executable variable for the checker, for easier
configuration purpose.  Nevertheless, using `flycheck-define-command-checker` is
the recommended way to define a checker with compile-time arguments.
